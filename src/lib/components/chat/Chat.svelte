<script lang="ts">
	...
	const recentlyAccepted = (accepted_at?: number): boolean => {
		...
		const epsilon = 5 * 60 // 5 minutes of leeway
		// counts as accepted when accpted within the last 24 hours
		if(nowInS - dayInS - epsilon < accepted_at && accepted_at < nowInS + epsilon) {
			return true
		}
	...
	const acceptHanlder = async () => {
		if($user){
			...
			if (res) {
				await user.set({...$user, accepted_at: new Date().getTime()/1000});
			}
		}
	};
