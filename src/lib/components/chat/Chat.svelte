<script lang="ts">
	...
	$: hasAccepted = $user?.accepted_at ? (new Date().getTime() > $user.accepted_at) : false
	...
	const disclaimerText = () => {
		return env.PUBLIC_DISCLAIMER_TEXT
	}
...
>
	{#if !loading}
		...
		<div class="flex flex-col px-[5%] pt-[30px] pb-[10px] gap-y-6 items-center">
			<h2 class="font-bold">User has not accepted yet</h2>
			<p>{@html disclaimerText()}</p>
			<button
